...
- Lines 116–122: every hook command must contain `$_C/plugins/marketplaces/bjlee2024/plugin` fallback (issue #1215).
- Lines 124–132: cache path must be tried BEFORE marketplace fallback (issue #1533).
- Lines 84–99: MCP launcher includes `.codex/plugins/cache/claude-mem-local/claude-mem` and `plugins/cache/bjlee2024/claude-mem` fallbacks; root and bundled launchers stay synced.
...
3. **Candidate enumeration + filter loop** (reuse the existing pipeline from `plugin/hooks/hooks.json:24`):
   ...
   _P=$({
     ...
     ls -dt "$_C/plugins/cache/bjlee2024/claude-mem"/[0-9]*/ 2>/dev/null;
     printf '%s\n' "$_C/plugins/marketplaces/bjlee2024/plugin";
  ...
  for (const file of ['plugin/hooks/hooks.json', 'plugin/hooks/codex-hooks.json']) {
    for (const command of commandHooksFrom(file)) {
      ...
      it(`[${file}] resolves _P from cache when CLAUDE_PLUGIN_ROOT is unset`, () => {
        // Set up tmp $HOME/.claude/plugins/cache/bjlee2024/claude-mem/12.0.0/plugin/scripts/<requireFile>
