 ...
 * failure the universal installer (`npx @bjlee2024/claude-mem install`) can hit, and the
...
const BUN_REMEDIATION = (ctx: RemediationContext): string =>
  ctx.platform === 'win32'
    ? 'Install Bun manually then re-run `npx @bjlee2024/claude-mem install`. Windows: `winget install Oven-sh.Bun` (or `powershell -c "irm bun.sh/install.ps1 | iex"`).'
    : 'Install Bun manually then re-run `npx @bjlee2024/claude-mem install`. macOS/Linux: `curl -fsSL https://bun.sh/install | bash` (or `brew install oven-sh/bun/bun`).';

const UV_REMEDIATION = (ctx: RemediationContext): string =>
  ctx.platform === 'win32'
    ? 'Install uv manually then re-run `npx @bjlee2024/claude-mem install`. Windows: `winget install astral-sh.uv` (or `powershell -c "irm https://astral.sh/uv/install.ps1 | iex"`).'
    : 'Install uv manually then re-run `npx @bjlee2024/claude-mem install`. macOS/Linux: `curl -LsSf https://astral.sh/uv/install.sh | sh` (or `brew install uv`).';
...
export const ERROR_CATEGORIES: ErrorCategory[] = [
  ...
  },
  {
    ...
    remediation: () =>
      'ERESOLVE peer-dependency conflict in marketplace deps that --legacy-peer-deps could not resolve. Open an issue at https://github.com/bjlee2024/claude-mem/issues with the conflicting peer ranges shown above.',
  },
  {
    ...
    remediation: () =>
      'bun install failed to resolve packages — check network connectivity and re-run `npx @bjlee2024/claude-mem install`. Cached packages in ~/.bun/install/cache will be reused.',
  },
  {
  ...
  },
  {
    ...
    remediation: () =>
      'Existing plugin.json is corrupt. Run `rm -rf ~/.claude/plugins/marketplaces/bjlee2024` and re-run `npx @bjlee2024/claude-mem install`.',
  },
  {
    id: 'all-ides-failed',
    severity: ErrorSeverity.ABORT,
  ...
  },
  {
    ...
    remediation: () =>
      'Re-run `npx @bjlee2024/claude-mem install --ide=<name>` to retry just this IDE. The captured stderr is shown above.',
  },
  {
  ...
  },
  {
    ...
    remediation: () =>
      'Could not verify the tool version after install — the installation is likely OK. Re-run `npx @bjlee2024/claude-mem install` if features misbehave.',
  },
  {
  ...
  },
  {
    ...
    match: () => true,
    remediation: (ctx) =>
      `An unexpected installer error occurred. Capture ${ctx.dataDir}/last-install-error.json and open an issue at https://github.com/bjlee2024/claude-mem/issues.`,
  },
];
