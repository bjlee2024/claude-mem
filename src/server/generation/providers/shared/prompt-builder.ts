...
export function buildServerGenerationPrompt(
  ...
  const prompt = [
    ...
    '',
    'LANGUAGE: Write every observation field (title, subtitle, facts, narrative,',
    'concepts) in Korean (한국어). Keep code identifiers, file paths, commands,',
    'and technical/library/API names verbatim in their original form (usually',
    'English) — do not translate or transliterate them. Do NOT use Chinese or',
    'Japanese; if the source events contain such text, render the meaning in',
    'Korean.',
    '',
