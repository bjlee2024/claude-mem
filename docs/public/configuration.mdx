...
| `me` | 현재 프로젝트에서 `git config user.name`으로 읽은 사용자 본인이 기록한 Observation만 주입 |
...
`me`로 설정했는데 현재 디렉터리에서 git user를 읽을 수 없는 경우(예: git 저장소가 아니거나
`user.name`이 설정되어 있지 않은 경우) 컨텍스트가 통째로 비는 것을 막기 위해 전원(필터 없음)으로
자동 폴백한다.
