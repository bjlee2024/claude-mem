...
- **세션 종료로는 풀리지 않는다.** 일시 중지를 해제하는 방법은 `/claude-mem:resume`을 실행하거나 24시간이 지나 자동 만료되는 것, 이 두 가지뿐이다. 세션이 끝난다고 저절로 풀리지 않으므로, 다음 세션에서도 기록을 켜고 싶다면 명시적으로 `/claude-mem:resume`을 실행해야 한다.
